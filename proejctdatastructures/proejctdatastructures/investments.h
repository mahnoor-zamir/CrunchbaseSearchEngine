...
// Class to represent a single investments.csv file
class investments
{
public:
    std::string funding_round_id;
    std::string funded_object_id;
    std::string investor_object_id;
};

HashTable<int, investments> parseInvestments()
{
    // Open the CSV file
    ...
    HashTable<int, investments> data(10000);
    ...
    while (file.good())
    {
        ...
        while (end != std::string::npos)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                record.funded_object_id = column_data;
                break;
            case 3:
                record.investor_object_id = column_data;
                break;
            }
    ...
    file.close();
    int key = 1234;
    investments row = data.Get(key);

    // Access the attributes of the record
    //std::cout << "Funds ID: " << row.funding_round_id << std::endl;
    //std::cout << "Object ID: " << row.funded_object_id << std::endl;
    // ...
